...
if (isMain) {
  ...
  switch (cli.command) {
    ...
    case "flow": {
      ...
      // Hardware Auto-Detection for Onboarding
      const os = await import("os");
      const isAppleSilicon = os.platform() === 'darwin' && os.cpus()?.[0]?.model?.includes('Apple');
      const totalRamGB = os.totalmem() / (1024 ** 3);
      
      let isLite = Boolean(cli.values.lite);

      if (!isLite && isAppleSilicon && totalRamGB < 16) {
        console.warn(`\n[HARDWARE DETECTED] Machine relies on unified Apple Silicon mapping (< 16GB total RAM).`);
        console.warn(`[SUGGESTION] Massive 4B parameter models will likely exhaust available VRAM and crash the embedding runtime.`);
        console.warn(`[AUTO-CONFIG] Forcefully engaging --lite mode (0.8B models) for safe operation on this hardware.\n`);
        isLite = true;
      } else if (isLite) {
